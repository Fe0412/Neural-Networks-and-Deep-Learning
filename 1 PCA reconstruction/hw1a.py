...
def plot_mul(c, D, im_num, X_mn, num_coeffs, n_blocks):
    ...
    pyplot.show()
    f.savefig('output1a/hw1a_{0}_im{1}.png'.format(n_blocks, im_num))
    plt.close(f)
...
def main():
    ...
    for sz, nc in  zip(szs, num_coeffs):
        ...
        # Function application
        X = window_function(Ims.reshape(no_image, 1,256,256))

        #Y = np.zeros(shape=(no_image * n_blocks_in_image, sz * sz))
        #i = 0
        #j = 0
        #for i in range(no_image * n_blocks_in_image):
        #    for j in range(sz * sz):
        #        Y[i][j] = Ims[i/(n_blocks_in_image)][(i%(sz * sz))/sz*sz + j/sz][i%sz*sz + j%sz]
        ...
        plot_top_16(D, sz, imname='output1a/hw1a_top16_{0}.png'.format(sz))
        print("one loop has finished")
