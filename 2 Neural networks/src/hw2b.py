...
class myMLP(object):
    ...
    def __init__(self, rng, input, n_in, n_hidden, n_out, n_layers,):
        ...
        temp = [self.hiddenLayer.output]
        absnum = 0
        sumnum = 0
        sumparams = self.hiddenLayer.params
        for i in range(n_layers):
            ...
            temp.append(self.hiddenLayer2.output)
            absnum = absnum + abs(self.hiddenLayer2.W).sum()
            sumnum =  sumnum + (self.hiddenLayer2.W ** 2).sum()
            sumparams = sumparams + self.hiddenLayer2.params

        # The logistic regression layer gets as input the hidden units
        # of the hidden layer
        finalinput = temp[len(temp) - 1]
        ...
        # made out of
        self.params = self.logRegressionLayer.params + sumparams
...
def test_mlp(learning_rate=0.01, L1_reg=0.00, L2_reg=0.001, n_epochs=1000,
             batch_size=20, n_hidden=100, verbose=True, n_layers = 1):
    """
    ...
    return 100 - test_score * 100.

if __name__ == '__main__':
    res = []
    layers = 2
    for n_la in range(0,layers):
        accuracy = test_mlp(learning_rate=0.01, L1_reg=0.00, L2_reg=0.0001, n_epochs=1000,
             batch_size=20, n_hidden=2, verbose=True, n_layers = n_la)
        res.append(accuracy)
    print(res)
