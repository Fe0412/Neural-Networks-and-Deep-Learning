...
class RNNSLU2(object):
    ...
    def __init__(self, nh, ng, nc, ne, de, cs, normal=True):
        ...
        self.params = [self.emb, self.wx, self.wh, self.w,
                       self.bh, self.b, self.h0, self.h1, self.wh2, self.bh2, self.wx2]
        ...
        def recurrence(x_t, h_tm1, g_tm1):
            h_t = T.nnet.sigmoid(T.dot(x_t, self.wx)
                                 + T.dot(h_tm1, self.wh) + self.bh)
            g_t = T.nnet.sigmoid(T.dot(h_t, self.wx2)
                                 + T.dot(g_tm1, self.wh2) + self.bh2)
            s_t = T.nnet.softmax(T.dot(g_t, self.w) + self.b)
            return [h_t, g_t, s_t]
